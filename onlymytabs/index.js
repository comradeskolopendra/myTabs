const openModalButtons = document.querySelectorAll('[data-open-dmodal]');
const closeModalButtons = document.querySelectorAll('[data-close-dmodal]');
const modalInnerContent = document.querySelector('.modal__inner');
const content = document.querySelectorAll('.content');
const nextButton = document.querySelector('#next');
const prevButton = document.querySelectorAll('.back');
let step = 1;

const toggleModal = (e) => {
    const target = e.currentTarget;
    const modalId = target.dataset.openDmodal || target.dataset.closeDmodal;
    const modal = document.querySelector(`[data-dmodal="${modalId}"]`);
    modal.classList.toggle('show');
}

const toogleTab = (step) => {
    content.forEach(item => item.classList.remove('active'));
    const currentContent = document.querySelector(`[data-content-active="${step}"`);
    currentContent.classList.add('active');
}

openModalButtons.forEach(button => button.addEventListener('click', (e) => toggleModal(e)))
closeModalButtons.forEach(button => button.addEventListener('click', (e) => toggleModal(e)))

modalInnerContent.addEventListener('click', (e) => e.stopPropagation());
...
prevButton.forEach(button => button.addEventListener('click', (e) => {
    if (step <= 1) {
        return
    } else {
        step -= 1;
